...
//  AR Ruler
//
//  Created by Emir Keleş on 15.04.2023.
//
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    func calculate() {
        ...
        var distance = sqrt(pow(start.position.x - end.position.x, 2) + pow(start.position.y - end.position.y, 2) +
             pow(start.position.z - end.position.z, 2))
        let tdistance = String(format: "%.f", distance*100)
        updateText(text: "\(tdistance)", atPosition: end.position)
